...
public class IOHandlerTest {
  ...
  @Test
  public void parse_unsignedChar_isNumeric() {
    Variant v = new Variant.Builder(StudioAPI.CDPValueType.eUCHAR).parse("255").build();
    assertEquals("eUCHAR parses the decimal 255", Integer.valueOf(255), v.getValue());
  }

  @Test
  public void createVariant_appliesClockDeltaToNonZeroTimestampsOnly() {
    // The clock delta applies to a non-zero remote timestamp only.
    long delta = 500_000;
    ...
    assertEquals(Instant.ofEpochSecond(0, 0), zeroTs.getTimestamp());
    ...
    assertEquals(Instant.ofEpochSecond(0, 1000 + delta), nonZeroTs.getTimestamp());
  }

  @Test
  public void setRemoteValue_narrowIntTypes_sendTheirValueOnWire() throws Exception {
    CapturingTransport transport = new CapturingTransport();
    IOHandler ioHandler = new IOHandler(transport);
  ...
  public void parse_negativeIntoUnsigned_throws() {
    // A negative string is no valid unsigned value, so parsing rejects it.
  ...
  public void repeatedAuthExpired_raisesOnePromptPerCycle() throws Exception {
    ...
    // Count only the REAUTHENTICATION_REQUIRED deliveries, the prompts. The granted result reaches this
    // requester too.
    ioHandler.setCredentialsRequester(result -> {
  ...
  public void reauthUsesLatestChallengeAfterSuppressedBurst() throws Exception {
    ...
    // is suppressed for prompting, and the re-auth still answers the latest challenge.
  ...
  public void encryptedPasswordReissueUsesLatestChallengeAfterSuppressedBurst() throws Exception {
    // The auto-answered EncryptedPassword reissue also answers the latest challenge, so a suppressed
    // burst (B) after the first answer (A) reissues for B.
    CapturingTransport transport = new CapturingTransport();
    IOHandler ioHandler = new IOHandler(transport);
    ioHandler.setTimeSyncEnabled(false);
    ioHandler.setCredentialsRequester(result -> { });
