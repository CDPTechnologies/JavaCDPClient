...
public class CompositeAuthRequestTest {
  ...
  @Test
  public void deliveredRequest_remembersTheAnsweringPrompt() {
    StubAuthRequest first = new StubAuthRequest();
    CompositeAuthRequest composite = new CompositeAuthRequest(first);

    composite.accept(new HashMap<>());

    assertSame(composite, first.answeringPrompt);
  }
  ...
  /** Minimal request that records accept/reject, the answering prompt, and returns a settable suggested-user list. */
  private static class StubAuthRequest implements ReauthRequest {
    boolean accepted;
    boolean rejected;
    ...
    @Override public CompositeAuthRequest getAnsweringPrompt() { return answeringPrompt; }
    @Override public void setAnsweringPrompt(CompositeAuthRequest prompt) { answeringPrompt = prompt; }
  }
}
