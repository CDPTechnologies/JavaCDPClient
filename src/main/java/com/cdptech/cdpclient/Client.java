...
public class Client implements Runnable {
   ...
   * @param endpointIdentificationEnabled When enabled, the client checks that the host it connects to matches the
   *                                      name in the certificate. The generated StudioAPI.crt names the system and
   *                                      the application, so leave this off when connecting by IP address.
   */
  @SneakyThrows
  ...
  public void setIgnoreCertificates(boolean ignoreCertificates) {
    // TrustingSSLSocketFactory's trust manager is an X509ExtendedTrustManager with empty checks, so the JDK
    // runs no endpoint identification for it and this path needs no parameter handler.
  ...
  private void handleReauthentications() {
    if (compositeReauthRequest != null && compositeReauthRequest.isReady() && compositeReauthRequest.isRejected()) {
      close();
    }
    dropExpiredPrompt();
  }

  /** An answered prompt past the cache window hands its answer to nobody. */
  private void dropExpiredPrompt() {
    if (compositeReauthRequest != null && compositeReauthRequest.isReady() && isCacheExpired(compositeReauthRequest)) {
  ...
  void requestReauthentication(ReauthRequest request) {
    // A request that starts a cycle joins the current prompt, and an answered prompt hands its answer
    // over. Any other code is the server's or this client's verdict on an answer this connection
    // received, so the connection joins the correction prompt already open, or takes the current
    // prompt's answer while it is a newer one, or opens the next prompt.
    dropExpiredPrompt();
    boolean startsCycle =
        request.getAuthResult().getCode() == AuthRequest.AuthResultCode.REAUTHENTICATION_REQUIRED;
    CompositeAuthRequest answered = request.getAnsweringPrompt();
    if (compositeReauthRequest != null && !compositeReauthRequest.isReady()) {
      compositeReauthRequest.add(request);
    } else if (compositeReauthRequest != null && startsCycle) {
      compositeReauthRequest.add(request);
    } else if (compositeReauthRequest != null && answered != null && compositeReauthRequest != answered) {
      compositeReauthRequest.add(request);
    } else {
  ...
  private static boolean isCacheExpired(CompositeAuthRequest prompt) {
    long cacheLength = Instant.now().getEpochSecond() - prompt.getReadyTimestamp().getEpochSecond();
    return cacheLength >= REAUTH_CACHE_LENGTH_SECONDS;
  }
